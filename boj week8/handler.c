...
{
int press; //key 값을 저장할 변수 press

press = ioread32(key_addr+EDGE);// press 의 주소 0xFF2005C (edgecapture register) (pushbutton 내용 저장할 주소)
iowrite32(press,key_addr+EDGE); //현재 press 0xFF2005C (edgecapture register) 토글 시키기전 press 값 보존 
value ^=press;  //press Xor 값을 value 에 저장 
iowrite32(value, ledr_addr); // value 를 LED 에 출력 
iowrite32(0xf, key_addr + EDGE); //edge capture 레지스터 활성화 
